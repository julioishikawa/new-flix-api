...
const { TMP_FOLDER, UPLOADS_USERS_FOLDER } = configureMulter();

export class DiskUsersStorage {
  async saveFile(file: any) {
    await fs.promises.rename(
      path.resolve(TMP_FOLDER, file),
      path.resolve(UPLOADS_USERS_FOLDER, file)
    );
  ...
  async deleteFile(file: any) {
    const filePath = path.resolve(UPLOADS_USERS_FOLDER, file);
